...
void preNtReadFile(CPUX86State *env, void* user_data)
{
    ...
    pReadData = (PNtReadWriteData)g_malloc(sizeof(NtReadWriteData));
    if(pReadData != NULL)
    {
...
void preNtWriteFile(CPUX86State *env, void* user_data)
{
    ...
    pWriteData = (PNtReadWriteData)g_malloc(sizeof(NtReadWriteData));
    if(pWriteData != NULL)
    {
...
void preNtSecureConnectPort(CPUX86State *env, void* user_data)
{
    ...
    pPortData = (PNtConnectPortData)g_malloc(sizeof(NtConnectPortData));
    if(pPortData != NULL)
    {
...
void preNtRequestWaitReplyPort(CPUX86State *env, void* user_data)
{
    ...
    {
        pPortData = (PNtRequestPortData)g_malloc(sizeof(NtRequestPortData));
        if(pPortData != NULL)
        {
...
void OnCsrWriteDataPre(CPUX86State *env, ULONG Message, ULONG VirtualOffset)
{
    ...
    buffer = (uint8_t *)g_malloc(cbSize + 1);
    if(buffer == NULL)
    {
...
void OnCsrReadDataPost(CPUX86State *env, ULONG Message, ULONG VirtualOffset)
{
    ...
    buffer = (uint8_t *)g_malloc(length + 1);
    if(buffer == NULL)
    {
